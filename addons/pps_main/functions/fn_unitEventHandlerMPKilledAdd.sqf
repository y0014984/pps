 ...
 * Author: y0014984
 *
 * Adds 'MPKilled' event handler to all units.
 *
 * Arguments:
 * 0: _unit <OBJECT>
 *
...
if (local _unit) then
{
	_index = _unit addMPEventHandler ["MPKilled",
	{
		...
		{				
			...
			if (local _unit) then
			{
				// _eventHandlerInformation = format ["Event Handler 'MPKilled' fired for local _unit.\n\n_unit: %1 (%2 - %3)\n_killer: %4 (%5 - %6)\n_instigator: %7 (%8 - %9)\nclientId: %10", _unit, _unitUid, _unitSide, _killer, _killerUid, _killerSide, _instigator, _instigatorUid, _instigatorSide, _clientId];
				// hint _eventHandlerInformation;
				// [_eventHandlerInformation] call PPS_fnc_log;
				...
				if (_unitUid != "") then
				{
					...
					[format ["[%1] PPS Event Handler 'MPKilled' fired for local _unit: %2 (%3)", serverTime, _key, _playerUid]] call PPS_fnc_log;
				};
			};
		};
	}];
